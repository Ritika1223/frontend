...
const Register = () => {
  ...
  if (!/^\d{10}$/.test(phone)) {
    ...
    try {
    const response = await axios.post(API_URLS.USER_LOGIN, { phone });
...
const handleVerify = async () => {
  ...
  try {
    const response = await axios.post(API_URLS.USER_LOGIN_OTP,{
      phone,
      otp
    });
      ...
      <div className="max-w-md w-full bg-white/70 backdrop-blur-lg p-8 rounded-2xl shadow-xl border border-white/20 space-y-6">
        ...
        <div className="text-center text-sm text-gray-600 animate-fade-up-delay">
          <p>
            ...
            <button onClick={() => navigate('/register')} className="font-medium text-[#3B4B96] hover:text-[#FF5722]">
              Register
            </button>
          </p>
        </div>
