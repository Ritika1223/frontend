...
const OperatorForm = () => {
  ...
  const addBus = () => {
    setFormData(prev => {
      if (prev.buses.length >= 3) return prev; // Prevent adding more than 3
      return {
        ...prev,
    ...
    setOpenBusTypeDropdown(prev => [...prev, false]);
    setOpenBusModelDropdown(prev => [...prev, false]);
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto pt-5 ">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-lg p-6 space-y-8">
          {/* Form Heading */}
          <h2 className="text-2xl font-bold text-center text-[#3B4B96]">
            Operator Registration Form
            <br />
            <span className="text-base font-normal text-black">
              Attach your buses, tempo travellers, minibuses, and cars. Please share the following details for us to onboard you.
            </span>
          </h2>
            ...
            <div className="bg-gray-50 p-4 sm:p-6 rounded-lg border border-gray-200 space-y-4 sm:space-y-6">
              ...
              <div className="space-y-6">
                {formData.buses.map((bus, index) => {
                  const selectedBusTypes = formData.buses
                    .map((b, i) => (i !== index ? b.busType : null))
                    .filter(Boolean);

                  const filteredBusTypeOptions = busTypeOptions.filter(
                    (type) => !selectedBusTypes.includes(type) || type === bus.busType
                  );
                  ...
                  return (
                    <React.Fragment key={index}>
                      ...
                      <div className="space-y-2">
                        ...
                        <OperatorDropdown
                          options={filteredBusTypeOptions}
                          value={bus.busType}
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                ...
                >
                  ...
                  <button
                    ...
                    className="text-[#3B4B96] underline hover:text-[#FF5722]"
                  >
                    terms
                  </button>.
                </label>
      ...
      ${isConfirmed && !isSubmitting
        ...
        {showTerms && (
          <div className="absolute inset-0 flex items-end justify-center bg-black/20 z-50 pb-28 sm:pb-36">
            <div className="bg-white border border-gray-300 shadow-xl rounded-xl w-[90%] sm:w-[600px] max-h-[60vh] overflow-y-auto p-6 relative">
              <h2 className="text-lg font-semibold text-center mb-4 text-[#3B4B96]">
                Terms and Conditions
              </h2>
              <div className="text-sm text-gray-700 space-y-3 leading-relaxed">
                <p>1. You agree to provide accurate business and vehicle information for listing on <strong>antbus.in</strong>.</p>
                <p>2. You must upload valid KYC documents (Aadhaar, PAN, etc.) which will be securely verified.</p>
                <p>3. AntBus reserves the right to approve or reject your registration based on document authenticity.</p>
                <p>4. You consent to receiving communication regarding bookings and operations related to your service.</p>
                <p>5. Any misuse of the platform or false information may result in deactivation or legal action.</p>
              </div>
              <div className="mt-6 text-right">
                <button
                  onClick={() => setShowTerms(false)}
                  className="px-4 py-2 rounded-md bg-[#3B4B96] text-white hover:bg-[#FF5722] transition-all duration-200"
                >
                  Close
                </button>
              </div>
